...
input_assoluto_path = os.path.join(script_dir, 'input.txt')
output_assoluto_path = os.path.join(script_dir, 'output.csv')


converti_file(input_assoluto_path, output_assoluto_path)
        # path_assoluto = os.path.abspath('_personale\nomi_data_nascita.txt')
print('SUCCESO: Il file è stato creato corettamente alla '
    f'posizione {output_assoluto_path}!'
)
